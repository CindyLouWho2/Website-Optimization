...
var pizzaElementGenerator = function(i) {
  ...
  pizzaImage.src = "images/pizza_small.png";
...
var resizePizzas = function (size) {
   ...
   function changePizzaSizes (size) {  //added at 3:35p on 4/15
    switch(size) {
      case "1":
        newWidth = 25;
        break;
 ...
 /* function determineDx (elem, size) {         //removed 4/15 - 3:34p
...
// Removed this whole section since
...
/*  var pizzaTop = last_known_scroll_position / 1250;                                                                  //same

 //var items = document.querySelectorAll('.mover');  // rmv 4/16
  var items = document.getElementsByClassName('mover'); //added 4/16 rmv 4/17                                       //same
  ...
  //  for (var i = 0; i < items.length; i++) {
      items[i].style.transform = "translateX(" + phase[i % 5] + "px)";
                                     ...
                                     //  var scrollPosition = document.body.scrollTop / 1250;//added 4/17 - rmv 4/22 6/3_ not supported in chrome anymore     //sane
                                    //var itemsLength = items.length;
                                     //for (var i = 0; i < items.length; i++) { //This line is causing 'forced reflow' bottleneck:
//  for (var i = 0, len = items.length, phase; i < len; i++) {
    //var items = document.getElementsByClassName('mover'); //aaded 4/17
    // document.body.scrollTop is no longer supported in Chrome.
   // var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
   //var phase = Math.sin((scrollTop / 1250) + (i % 5)); //rmv 4/17
   //var scrollPosition = document.body.scrollTop / 1250;
//       var phase = Math.sin((scrollPosition) + (i % 5)); //added 4/17 - rmv 4/22 // offset that allows the pizzas to move
//    items[i].style.left = items[i].basicLeft + 100 * phase + 'px';      //rmv 5/30 but only 3 background pizza's
   // items[i].style.transform = "translateX(" + 100 * phase[i % 5] + "px)";
//  }
                                   // User Timing API to the rescue again. Seriously, it's worth learning.
                                      // Super easy to create custom metrics.
  window.performance.mark("mark_end_frame");                                                                        //same
 window.performance.measure("measure_frame_duration", "mark_start_frame", "mark_end_frame");                       //same
  if (frame % 10 === 0) {                                                                                         //same
   var timesToUpdatePosition = window.performance.getEntriesByName("measure_frame_duration");                      //same
  logAverageFrame(timesToUpdatePosition);                                                                         //same
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var movingPizzas = document.getElementById('movingPizzas1');//added 4/17

  for (var i = 0; i < 30; i++) {            //was 200 on 5/30
    var elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza_small.png";
    elem.style.height = "100px";
    elem.style.width = "73.333px";
    elem.basicLeft = (i % cols) * s;  // column width
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    /*document.querySelector("#movingPizzas1")*/ movingPizzas.appendChild(elem);
  }
  updatePositions();
});
